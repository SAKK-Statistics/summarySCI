...
The function `summaryByVisit()` produces a table with descriptive statistics for continues and categorical variables at different time points (visits). It is largely based on the function `gtsummary::tbl_summary()` and `gtsummary::tbl_strata_nested_stack`. The changes as
...
dataset. We have three hypotetical lab-values: LDH, ANC and Lymphocytes and two categorical variables, grade and stage. Each lab-value has been measured at 10 time points (visits), which can further be divided into three visit groups (baseline, treatment and follow-up). In addition, we have a grouping variable called 'arm'.
...
Now, we use `summarySCI::summaryByVisit` to create a summary table for median and range of each lab-value by visit.

```{r}
summaryByVisit(data = data,
