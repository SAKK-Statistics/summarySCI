...
Creates publication-ready summary tables for categorical data grouped,
by visit

## Usage

``` r
summaryByVisitCategorical(
  data,
  vars = NULL,
  group = NULL,
  labels = NULL,
...
- vars:
  ...
  specified with quotes, e.g. `"grade"` or `c("grade", "stage")`.
  Default to all variables present in the data except `group`.
...
- digits_cat:
  ...
  Digits for summary statistics of categorical variables. Default to 1.
